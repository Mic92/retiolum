...
let {
  body = netname: subnetname: suffixSpec: rec {
    address =
      ...
      in
      normalize-ip6-addr (checkAddress addressLength (joinAddress subnetPrefix suffix'));
    addressCIDR = "${address}/${toString addressLength}";
    addressLength = 128;

    inherit netname;
    netCIDR = "${netAddress}/${toString netPrefixLength}";
    netAddress = normalize-ip6-addr (appendZeros addressLength netPrefix);
    netHash =
      toString
        {
          retiolum = 0;
    ...
    subnetAddress = normalize-ip6-addr (appendZeros addressLength subnetPrefix);
    ...
    suffix = getAttr (builtins.typeOf suffixSpec) {
      set = concatStringsSep ":" (
        stringToGroupsOf 4 (hashToLength (suffixLength / 4) suffixSpec.hostName)
      );
      string = suffixSpec;
    };
  ...
  checkAddress =
    ...
    in
    assert (
      check "address malformed; lone leading colon: ${addr}" (
        head parsedaddr == "" -> tail (take 2 parsedaddr) == ""
      )
    );
    assert (
      check "address malformed; lone trailing colon ${addr}" (
        last parsedaddr == "" -> head (takeLast 2 parsedaddr) == ""
      )
    );
    assert (
      check "address malformed; too many successive colons: ${addr}" (
        length (filter (x: x == "") normalizedaddr) > 1 -> addr == [ "" ]
      )
    );
    assert (check "address malformed: ${addr}" (all (test "[0-9a-f]{0,4}") parsedaddr));
    assert (check "address is too long: ${addr}" (length normalizedaddr * 16 <= maxaddrlen));
    addr;
  ...
  joinAddress =
    ...
    let
      ...
      delimiter = optional (
        length (normalizePrefix ++ normalizeSuffix) < 8
  ...
  normalize-ip6-addr =
    let
      ...
      group-zeros =
        ...
        if max.pos == 0 then
          ...
          let
            sep = if 8 - (length lhs + length rhs) == 1 then ":0:" else "::";
          in
    ...
    in
    a: toLower (if test ".*::.*" a then a else group-zeros (drop-leading-zeros a));
    ...
    let
      acc =
        foldl'
          (
            acc: c:
            if stringLength acc.chunk < n then
              {
                chunk = acc.chunk + c;
                chunks = acc.chunks;
              }
